...
class Person {
    ...
    this.attitudeTasks = attitudeTasks;
     var task;
      this.attitudeTasks.forEach(function (itemAT) {
        ...
        task = context.getATTById(itemAT['task'].id);
        this[_totalXPpoints] += parseInt(task.points);
  ...
  addAttitudeTask(taskInstance) {
    ...
    context.notify('Added Task to ' + this.name + ',' + this.surname, this.surname + ' ,' + this.name);
    context.getTemplateRanking();
  }
